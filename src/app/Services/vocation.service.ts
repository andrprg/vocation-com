...
export class VocationService {
  ...
  getCountDaysInFullMonth(months: Month[]): number {
    const countMonths = months.filter(value => value.excludeCountDay === 0)
      .length;
  ...
  getCountDaysInNotFullMonth(months: Month[]): number {
    const countDayInMonths = months
      ...
      .reduce((acc, value) => {
        ...
        const ret =
          (this.AVERAGE_DAYS_WORK * rb) / moment(value.month).daysInMonth();
        return (acc += ret);
      }, 0);
  ...
  getResult(vocation: Vocation, month: Month[]): Observable<Result> {
    return Observable.create(observer => {
      ...
      const znam =
        this.getCountDaysInFullMonth(month) +
        this.getCountDaysInNotFullMonth(month);
